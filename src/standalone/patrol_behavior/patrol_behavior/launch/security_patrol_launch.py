...
def generate_launch_description() -> LaunchDescription:
    ...
    robot_urdf = os.path.join(robot_desc_dir, 'urdf', 'standard', 'turtlebot4.urdf.xacro')
    ...
    remove_temp_sdf_file = RegisterEventHandler(event_handler=OnShutdown(
        on_shutdown=[
            OpaqueFunction(function=lambda _: os.remove(world_sdf_path) if os.path.exists(world_sdf_path) else None)
        ]))
    ...
    set_env_vars_resources2 = AppendEnvironmentVariable(
            'GZ_SIM_RESOURCE_PATH',
            str(Path(os.path.join(robot_desc_dir)).parent.resolve()))
