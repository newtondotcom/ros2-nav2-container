...
# Add undocking at the start of patrol and docking at the end
...
def main():
    ...
    print(
        f"Initial pose set to: x={initial_pose.pose.position.x}, y={initial_pose.pose.position.y}"
    )
    ...
    if route_name in routes:
        ...
        while rclpy.ok():
            ...
            print(
                f"Route direction: {'Forward' if route_direction == 1 else 'Reverse'}"
            )
            print(f"{'=' * 60}")
            ...
            for idx, waypoint in enumerate(current_waypoints):
                ...
                print(
                    f"Position: x={waypoint.pose.position.x:.2f}, y={waypoint.pose.position.y:.2f}"
                )
